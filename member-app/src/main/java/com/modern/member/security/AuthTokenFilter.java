...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    private void authenticateUser(HttpServletRequest request, String jwt) {
        ...
        int tokenVersion = jwtUtils.getVersionFromJwtToken(jwt);

        UserDetails userDetails = userDetailsService.loadUserByUsername(username);

        if (userDetails instanceof UserDetailsImpl customUserDetails) {
            if (customUserDetails.getTokenVersion() != tokenVersion) {
                log.warn("Security Alert: Token version mismatch for user {}. Token invalidated.", username);
                return;
            }
        }
